...
def resolve_chrome_exe(chrome_exe: str | None = None) -> str:
    playwright_candidates = glob.glob("/ms-playwright/chromium-*/chrome-linux*/chrome")
    candidates = [
        chrome_exe,
        CHROME_EXE,
    ...
    raise FileNotFoundError(
        "未找到 Chrome/Chromium，请用 --chrome-exe 或 SWU_CHROME_EXECUTABLE 指定路径"
    )


def is_safe_profile_dir(path: str) -> bool:
    profile = os.path.abspath(path)
    allowed_roots = {os.path.abspath(tempfile.gettempdir()), os.path.abspath(RUNTIME_DIR)}
    allowed_names = {"ChromeSwuLoginProfile", "browser-profile"}
    if os.path.basename(profile) not in allowed_names:
        return False
    for root in allowed_roots:
        try:
            if os.path.commonpath([profile, root]) == root and profile != root:
                return True
        except ValueError:
            continue
    return False
